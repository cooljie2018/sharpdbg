...
public class JustMyCodeTests(ITestOutputHelper testOutputHelper)
{
	...
	public async Task JustMyCode_StackTraceRequest_ReturnsNonUserSourceFramesAsExternalCode()
	{
		...
		stackTraceResponse.StackFrames.Should().BeEquivalentTo(expectedStackFrames, options => options.Excluding(s => s.Id).Excluding(s => s.Source.Checksums).Excluding(s => s.Source.VsSourceLinkInfo).Excluding(s => s.InstructionPointerReference));
		;
	}
}
